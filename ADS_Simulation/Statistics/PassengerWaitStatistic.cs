...
namespace ADS_Simulation.Statistics
{
    class PassengerWaitStatistic : Statistic
    {
        ...
        int[] latestPassengerArrival;
        int[] latestPassengerCount;
        int[] latestLongestWaiting;
        ...
        public PassengerWaitStatistic(int startTime, int endTime, int stationCount) : base(startTime, endTime)
        {
            ...
            latestPassengerArrival = new int[stationCount];
            latestPassengerCount = new int[stationCount];
            latestLongestWaiting = new int[stationCount];

        }

        public override void measure(State state, Event currentEvent)
        {
            if (currentEvent is TramDeparture td)
                OnDeparture(td.stationIndex);
            else if (currentEvent is DepartureStartstation ds)
                OnDeparture(ds.station.index);
            ...
            {
                totalPassengers[pa.stationIndex]++;
                int queueLength = state.stations[pa.stationIndex].waitingPassengers.Count;
                totalWaitingTime[pa.stationIndex] += timeDelta * (queueLength - 1); // Minus arrived passenger (has been waiting for 0)
                longestQueue = Math.Max(longestQueue, queueLength);

                latestPassengerArrival[pa.stationIndex] = state.time;
                latestPassengerCount[pa.stationIndex] = queueLength;
                latestLongestWaiting[pa.stationIndex] = Math.Min(
                    latestLongestWaiting[pa.stationIndex],
                    state.stations[pa.stationIndex].waitingPassengers.Peek());
            }

            void OnDeparture(int stationIndex)
            {
                int leftWaiting = state.stations[stationIndex].waitingPassengers.Count();
                totalPassengersLeftWaiting[stationIndex] += leftWaiting;
                mostPassengersLeftWaiting[stationIndex] = Math.Max(mostPassengersLeftWaiting[stationIndex], leftWaiting);

                int timeSinceLastArrival = state.time - latestPassengerArrival[stationIndex];
                int lastQueueLength = latestPassengerCount[stationIndex];
                totalWaitingTime[stationIndex] += timeSinceLastArrival * lastQueueLength;

                int longestWaiting = latestLongestWaiting[stationIndex];
                if (longestWaiting != int.MaxValue)
                    longestWaitTime[stationIndex] = Math.Max(longestWaitTime[stationIndex], state.time - longestWaiting);

                // Reset for next passengerarrival
                latestLongestWaiting[stationIndex] = int.MaxValue;
            }
            ...
            //        int waitingTime = WaitingTimeNewPassengers(station.waitingPassengers, queueDelta, state.time)
        ...
        private int WaitingTimeNewPassengers(Queue<int> queue, int queueDelta, int time)
        {
            int sum = 0;
            int idx = 0;
            int startFrom = queue.Count - queueDelta;
            foreach (int arrival in queue)
            {
                if (idx < startFrom)
                    continue;
                sum += time - arrival;
